...
| CentOS 9.0 及以上             | 命令方式<br/>1、创建 `lo` 网络连接：<br/>bash<br/>nmcli connection add con-name lo type loopback ifname lo connection.autoconnect yes<br/>如果系统中已存在 `lo` 网络连接，则无需重复执行此步骤。 | 命令方式<br/>nmcli connection modify lo +ipv4.addresses $VIP/32<br/>nmcli connection modify lo connection.autoconnect yes<br/>nmcli connection reload<br/>nmcli connection up lo<br/>UserData中添加以下内容：<br/>\#!/bin/bash<br/>nmcli connection modify lo +ipv4.addresses $VIP/32<br/>nmcli connection modify lo connection.autoconnect yes<br/>nmcli connection reload<br/>nmcli connection up lo |
